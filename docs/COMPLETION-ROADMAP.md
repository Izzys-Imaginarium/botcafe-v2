...
- [x] **Dynamic max output tokens** - Stream handler uses `getMaxOutputTokens(model)` instead of hardcoded 2048, fixing truncated responses on GLM and other providers. ✅
...
### **PHASE 9C: Context Builder Fixes & World Book Import** 🆕 ✅ **COMPLETED (2026-02-14)**
- [x] **Context Builder: Missing Bot Fields** — Added `gender`, `age`, and `behavior_settings.knowledge_sharing` to system prompt injection ✅
- [x] **Context Builder: Missing Persona Field** — Added `custom_instructions` injection after interaction preferences ✅
- [x] **Context Builder: Per-Message Persona Attribution** — Each message now labeled with the correct persona name from `message_attribution.persona_id` instead of using a single persona for all messages ✅
- [x] **Context Builder: Persona Switch Detection** — Detects distinct personas in the message window and adds a switch note to the system prompt ✅
- [x] **SillyTavern World Book Import** ✅
  - [x] Created `src/lib/tavern-card/world-book.ts` — Parser with validation, field mapping functions (`mapWorldBookPosition`, `mapWorldBookRole`, `mapWorldBookSelectiveLogic`, `getWorldBookActivationMode`, `getWorldBookSummary`) ✅
  - [x] Added `WorldBook` and `WorldBookEntry` types to `src/lib/tavern-card/types.ts` ✅
  - [x] Created `POST /api/knowledge-collections/import-worldbook` — Standalone endpoint (no bot linkage), full field mapping, auto-vectorization ✅
  - [x] Created `ImportWorldBookDialog` component with file preview showing entry names, keywords, and activation modes ✅
  - [x] Integrated import button into Lore Library page next to "New Tome" ✅
