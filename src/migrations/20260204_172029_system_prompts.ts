...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`system_prompts\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`system_prompts_updated_at_idx\` ON \`system_prompts\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`system_prompts_created_at_idx\` ON \`system_prompts\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`system_prompts_id\` integer REFERENCES system_prompts(id);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_system_prompts_id_idx\` ON \`payload_locked_documents_rels\` (\`system_prompts_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`system_prompts\`;`)
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`creator_follows_id\`) REFERENCES \`creator_follows\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "bot_id", "bot_interactions_id", "api_key_id", "mood_id", "knowledge_id", "knowledge_collections_id", "knowledge_activation_log_id", "conversation_id", "message_id", "memory_id", "vector_records_id", "token_gifts_id", "subscription_payments_id", "subscription_tiers_id", "token_packages_id", "personas_id", "creator_profiles_id", "creator_follows_id", "access_control_id", "self_moderation_id", "usage_analytics_id", "memory_insights_id", "persona_analytics_id", "legal_documents_id", "user_agreements_id", "documentation_id", "tutorials_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "bot_id", "bot_interactions_id", "api_key_id", "mood_id", "knowledge_id", "knowledge_collections_id", "knowledge_activation_log_id", "conversation_id", "message_id", "memory_id", "vector_records_id", "token_gifts_id", "subscription_payments_id", "subscription_tiers_id", "token_packages_id", "personas_id", "creator_profiles_id", "creator_follows_id", "access_control_id", "self_moderation_id", "usage_analytics_id", "memory_insights_id", "persona_analytics_id", "legal_documents_id", "user_agreements_id", "documentation_id", "tutorials_id" FROM \`payload_locked_documents_rels\`;`)
